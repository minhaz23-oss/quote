...
const Page = () => {
  ...
  const handleDelete = async (index) => {
     try {
      const confirmed = window.confirm('Are you sure you want to delete the quote??')
      if(confirmed){

        const res = await axios.delete(`/api/users/deletequote/${index}`);
        if (res.status === 201) {
          const updatedQuotes = [...userInfo.quotes];
          updatedQuotes.splice(index,1);
          setUserInfo({...userInfo,quotes: updatedQuotes})
        }
      }
     } catch (error) {
      console.log(error)
  ...
  return (
    <div className="flex flex-col px-[30px] md:px-[100px] py-[50px] items-center w-full min-h-screen">
      ...
      <Link
        href="/createquote"
        className="px-7 py-2 bg-white/5 border border-white text-white font-semibold rounded-md mt-3"
      >
        Create a quote
      </Link>
      <div className="w-full border-t border-white mt-4 bg-white/5 min-h-[300px] px-10 md:px-20 py-10">
        ...
        <div className=" w-full flex gap-[30px] flex-wrap">
          {userInfo &&
            userInfo.quotes.map((value, index) => (
              value && (
<div className=" w-[400px] min-h-[100px] bg-white text-black border border-white rounded-[10px] p-3 text-center  flex flex-col justify-center items-center relative">
          
          <h1>{value}</h1>
        
        <div className=" w-fit h-fit py-2 px-4 bg-black text-white rounded-md mt-3">
          - {userInfo.username}
        </div>
        <div className=" flex gap-[15px] absolute bottom-[10px] right-[10px] items-center">

        <div>
          <Link href={`/editquote/${index}`}>
            <FaEdit />
          </Link>
        </div>
